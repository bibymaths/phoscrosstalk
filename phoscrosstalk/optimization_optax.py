...
def run_single_optimisation_optax(
    ...
    *,
    ...
    learning_rate : float
        Step size for ``"adam"`` / ``"sgd"``; ignored for ``"lbfgs"``
        (L-BFGS uses a line-search-controlled step).
    optimizer_kind : {"adam", "sgd", "lbfgs"}
        Optax optimiser to use.
    log_every : int
        Log diagnostics every *log_every* steps (only when *verbose=True*).
    convergence_tol : float
        Stop early if the absolute change in loss falls below this threshold.
