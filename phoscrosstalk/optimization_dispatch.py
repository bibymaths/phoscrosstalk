...
"jaxopt_lbfgsb"    optimization_jaxopt.py    SciPy L-BFGS-B via JAXopt
"jaxopt_pgd"       optimization_jaxopt.py    Projected GD via JAXopt
"scipy_jax"        optimization_scipy_jax.py BFGS + reparameterisation
"scipy_jax_pen"    optimization_scipy_jax.py BFGS + quadratic penalty
"optax_adam"       optimization_optax.py     Adam + projection_box
"optax_sgd"        optimization_optax.py     SGD  + projection_box
"optax_lbfgs"      optimization_optax.py     L-BFGS + projection_box
"mpax"             optimization_mpax.py      SQP with MPAX QP subproblem
...
def _import_jaxopt_runner():
    from phoscrosstalk.optimization_jaxopt import run_single_optimisation_jaxopt
    return run_single_optimisation_jaxopt


def _import_scipy_jax_runner():
    from phoscrosstalk.optimization_scipy_jax import run_single_optimisation_scipy_jax
    return run_single_optimisation_scipy_jax


def _import_optax_runner():
    from phoscrosstalk.optimization_optax import run_single_optimisation_optax
    return run_single_optimisation_optax


def _import_mpax_runner():
    from phoscrosstalk.optimization_mpax import run_single_optimisation_mpax
...
def dispatch_optimisation(
    ...
    **kwargs,
    ...
    loss_fn : callable
        ``(theta, args) -> (scalar_loss, (f1, f2, f3, f4))`` closure from
        ``make_loss_fn`` in ``optimization.py``.
    theta0 : np.ndarray
        Starting parameter vector.
    xl, xu : np.ndarray
        Bounds from ``create_bounds()`` / ``problem.xl`` / ``problem.xu``.
    **kwargs
        Forwarded to the underlying solver.  Backend-specific keys (e.g.
        ``solver_kind``, ``optimizer_kind``) are injected automatically from
