...
Quadratic Programming (SQP) outer loop:
    ...
    For k = 0, 1, …, max_sqp_steps:
        1. Compute gradient g = ∇loss(θ_k)  and
                  Hessian H = ∇²loss(θ_k)  (via jax.hessian).
        2. Regularise: H_reg = H + δI  to ensure positive-definiteness.
        3. Solve the local QP:
               min_{Δθ}  ½ Δθᵀ H_reg Δθ + gᵀ Δθ
               s.t.      max(xl − θ_k, −r) ≤ Δθ ≤ min(xu − θ_k, r)
           where r = trust_radius (trust-region constraint on step size).
        4. Update: θ_{k+1} = clip(θ_k + Δθ, xl, xu).
...
---------------------------------
    ...
    # result.dual_solution    – dual variables
    # result.termination_status – convergence flag
...
⚠️  The full Hessian ``jax.hessian`` is O(n²) memory and O(n³) compute.
...
def run_single_optimisation_mpax(
    ...
    *,
    ...
    xl, xu : np.ndarray
        Lower/upper bounds.  Sourced from ``problem.xl`` / ``problem.xu``
        (output of ``create_bounds()``).
    max_sqp_steps : int
        Number of outer SQP iterations.
    eps_abs, eps_rel : float
        MPAX raPDHG absolute/relative tolerance for the inner QP solve.
    trust_radius : float
        Maximum step magnitude per SQP iteration (Δθ clipped to ±trust_radius).
    hess_reg : float
        Diagonal regularisation added to the Hessian; ensures PD.
    diagonal_hessian : bool
        If True, use only the diagonal of the Hessian (memory-efficient for
        large networks).
    finetune : bool
        Run a BFGS fine-tuning step (``jax.scipy.optimize.minimize``) after
        the SQP loop, with the reparameterised bounds strategy.
    finetune_max_steps : int
        Max BFGS iterations for fine-tuning.
    finetune_gtol : float
        Gradient tolerance for fine-tuning BFGS.

    Returns
    -------
    theta_opt : np.ndarray  (float64)
    total_loss : float
    f1, f2, f3, f4 : float
    """
    try:
