...
class TestDiffraxSimulation:
    def test_output_shape_dist(self):
        ...
        P_sim, A_sim = simulate_ode(
            ...
            m["Cg"], m["Cl"], m["site_prot_idx"],
            m["K_site_kin"], m["R"], m["L_alpha"],
            m["kin_to_prot_idx"], m["receptor_mask_prot"], m["receptor_mask_kin"],
            mechanism="dist",
        )
    ...
    def test_requires_modeldims(self):
        ...
        try:
            with pytest.raises(RuntimeError, match="ModelDims have not been set"):
                simulate_ode(
                    np.array([0.0, 1.0]),
                    np.zeros((2, 2)), np.zeros((2, 2)), np.zeros(10),
...
class TestOptimistixOptimisation:
    def test_loss_decreases(self):
        ...
        from phoscrosstalk.optimization import make_loss_fn, run_single_optimisation, create_bounds
...
class TestDependencyRegression:
    ...
    def test_no_scipy_ode_imports(self):
        ...
        assert "odeint" not in content, (
            "simulation.py still references odeint"
        )
    ...
    def test_package_importable(self):
        import phoscrosstalk
        from phoscrosstalk.simulation import simulate_ode, simulate_p_scipy
        from phoscrosstalk.optimization import NetworkProblem, NetworkOptimizationProblem
...
class TestCLIRegression:
    ...
    def test_new_flags_accepted(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", [
            ...
            "--rtol", "1e-5",
            "--atol", "1e-8",
            "--loss-weight-phospho", "2.0",
            "--loss-weight-abundance", "1.0",
            "--loss-weight-reg", "0.5",
        ])
        from phoscrosstalk.main import main
        with pytest.raises((SystemExit, Exception)) as exc_info:
...
class TestJaxMechanisms:
    ...
    def test_make_rhs_output_shape(self):
        ...
        rmp       = jnp.zeros(K)
        rmk       = jnp.zeros(M)
        prev_spi  = jnp.array(compute_prev_site_idx(np.array([0, 0, 1, 1]), N), dtype=jnp.int32)
